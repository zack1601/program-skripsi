...
def _build_cached_folium_map(df_disp):
    """
    Generates and returns HTML string representation of the Folium map.
    Cached via @st.cache_data for lightning-fast UI reruns.
    """
    if df_disp.empty or not df_disp['lat'].notnull().any():
        return None
    ...
    if coordinates_list:
        m.fit_bounds(coordinates_list, padding=(30, 30))
...
def render_map(df_filtered):
    ...
    Uses cached map HTML generation for ultra-fast UI performance.
    """
    ...
    if not df_filtered.empty:
        ...
        if df_disp['lat'].notnull().any():
            map_html = _build_cached_folium_map(df_disp)
            if map_html:
